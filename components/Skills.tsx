...
type CategoryId = 'frontend' | 'backend' | 'tools';
...
const Skills: React.FC = () => {
  ...
  const skillsData = {
    frontend: [
      { name: 'React.js', icon: <Code2 size={28} />, desc: 'Core Library' },
      { name: 'Next.js', icon: <Globe size={28} />, desc: 'React Framework' },
      { name: 'TypeScript', icon: <Code2 size={28} />, desc: 'Type Safety' },
      { name: 'Tailwind CSS', icon: <Layout size={28} />, desc: 'Modern Styling' },
      { name: 'Redux', icon: <Layers size={28} />, desc: 'State Management' },
      { name: 'Framer Motion', icon: <Box size={28} />, desc: 'Animations' },
      { name: 'HTML5/CSS3', icon: <Layout size={28} />, desc: 'Web Basics' },
      { name: 'PWA', icon: <Smartphone size={28} />, desc: 'Web Apps' },
    ],
    backend: [
      { name: 'Node.js', icon: <Server size={28} />, desc: 'Runtime' },
      { name: 'Express', icon: <Globe size={28} />, desc: 'Web Framework' },
      { name: 'PostgreSQL', icon: <Database size={28} />, desc: 'Database' },
      { name: 'Supabase', icon: <Database size={28} />, desc: 'Backend as Service' },
      { name: 'GraphQL', icon: <Box size={28} />, desc: 'API Query' },
      { name: 'REST API', icon: <Globe size={28} />, desc: 'API Standard' },
    ],
    tools: [
      { name: 'Git', icon: <GitBranch size={28} />, desc: 'Version Control' },
      { name: 'Docker', icon: <Container size={28} />, desc: 'Containerization' },
      { name: 'Linux', icon: <Terminal size={28} />, desc: 'OS' },
      { name: 'Figma', icon: <Layout size={28} />, desc: 'UI Design' },
      { name: 'Vite', icon: <Cpu size={28} />, desc: 'Build Tool' },
      { name: 'Jest', icon: <Command size={28} />, desc: 'Testing' },
    ]
  };

  return (
    <section id={SectionId.SKILLS} className="py-32 bg-slate-950 relative overflow-hidden">
      {/* Background Glow */}
      ...
      <div className="max-w-5xl mx-auto px-4 relative z-10">
        ...
        >
          <h2 className="text-3xl md:text-5xl font-black text-slate-100 mb-6 tracking-tight">Technical Skills</h2>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto font-light">
            The set of tools and technologies I use to create digital products.
          </p>
        </motion.div>
          ...
          <AnimatePresence mode="wait">
            {skillsData[activeTab].map((skill, index) => (
              ...
              >
                ...
                <div>
                  <h3 className="text-slate-200 font-bold mb-1">{skill.name}</h3>
